...
module Avg_Pool_1(
    input signed [197*16 -1:0] matrix,     // [MSR_M+MRB_S]*16 o/p
    input clk,
    output reg signed [195*16 -1:0] result // (200-3+1)*16=3168 output values, 16 bits per entry
    );
...
//    reg signed [6175:0] result;
    parameter MATRIX_ROWS_AVG1 = 1;
    parameter MATRIX_COLS_AVG1 = 197;
    parameter FILTER_ROWS_AVG1 = 1;
    parameter FILTER_COLS_AVG1 = 3;
    parameter RESULT_ROWS_AVG1 = 2;
    parameter RESULT_COLS_AVG1 = MATRIX_COLS_AVG1 - FILTER_COLS_AVG1 + 1;
    parameter ELEMENTS = FILTER_ROWS_AVG1 * FILTER_COLS_AVG1;
    ...
    always @(posedge clk) begin
        ...
        for (k = 0; k < 200; k = k + 1) begin // Number of subfilters
//            $display("MATRIX[%0d]: %d %d",k*16, k*16+16 , $signed(matrix[ 16*k + : 16]));
        end


        for (i = 0; i < RESULT_COLS_AVG1; i = i + 1) begin  // Limit to 193 iterations
//        for (i = 0; i < 5; i = i + 1) begin  // Limit to 193 iterations
            ...
            for (j = 0; j < FILTER_COLS_AVG1; j = j + 1) begin
